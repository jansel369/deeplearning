...
def batch_norm_grad_a():
    ...
    def bn_grad_i(optimizer, to_avg):
        def bn_grad_backward(dZ_tilda, param_grad, cache, parameters):
            ...
            dvar =  -0.5 * (dZ_norm * mu_dev * (gamma_i ** 3)).sum(1, True)
            dmu =  (dZ_norm * (-gamma_i)).sum(1, True) - 2 * to_avg * dvar * mu_dev.sum(1, True)
...
def bn_param_grad_a():
    """ Backprop functions that calculate grad dgamma & dbeta
    """
